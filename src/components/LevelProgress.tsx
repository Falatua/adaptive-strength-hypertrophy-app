 ...
 * means nothing on its own, so the words Forge level always travel with it, and the bar shows how much work
  ...
  return (
    <div className={`level-progress ${compact ? 'level-progress--compact' : ''}`}>
      <div className="level-progress__head">
        <strong>Forge level {progress.level}</strong>
      ...
      <div
        ...
        aria-label={`Forge journal level ${progress.level}, ${percent} percent toward Forge level ${progress.level + 1}`}
      >
        <i style={{ width: `${percent}%` }} />
      </div>
      <small>{progress.pointsIntoLevel.toLocaleString()} / {progress.pointsForNextLevel.toLocaleString()} journal points · {remaining.toLocaleString()} to Forge level {progress.level + 1}</small>
    </div>
  )
}
