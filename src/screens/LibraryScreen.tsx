      ...
      <Modal open={Boolean(selected)} onClose={() => setSelected(null)} title={selected?.name ?? 'Movement'} description={selected?.description} wide>
        {selected && (
          <div className="exercise-detail">
            ...
            {supportsBenchAngle(selected) && <section className="angle-history"><div className="panel__header"><div><p className="eyebrow">Setup-specific evidence</p><h3>Progress by bench angle</h3></div><Target size={18} /></div><p>Each recorded degree keeps its own progression and PR lane. Untracked older work stays visible but is never assumed to match a tracked setup.</p>{selectedAngleSummary.length ? <div className="angle-history__grid">{selectedAngleSummary.map((item) => <article key={item.label}><strong>{item.label}</strong><span>{item.sets} set{item.sets === 1 ? '' : 's'} · {item.volume.toLocaleString()} volume</span><small>Last used {new Date(item.latestAt).toLocaleDateString()}</small></article>)}</div> : <div className="compact-empty"><Target size={24} /><strong>No angle evidence yet</strong><p>Open Bench angle during a workout whenever the setup matters.</p></div>}</section>}
