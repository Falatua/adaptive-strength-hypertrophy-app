...
describe('training-block blueprint', () => {
  ...
  it('shows the weekly movement map and whole-block recovery route before editing', () => {
    ...
    const queueToggle = screen.getByRole('button', { name: 'Show the upcoming session queue' })
    expect(queueToggle).toHaveAttribute('aria-expanded', 'false')
    fireEvent.click(queueToggle)
    fireEvent.click(screen.getAllByRole('button', { name: 'Preview' })[0])
    const workoutPreview = screen.getByRole('dialog', { name: `Preview ${sessions[0].title}` })
    expect(workoutPreview).toHaveTextContent('Planned:')
    expect(workoutPreview).toHaveTextContent('Last:')
    expect(workoutPreview).toHaveTextContent('Preview only')
    fireEvent.click(within(workoutPreview).getByRole('button', { name: 'Close preview' }))
    expect(workoutPreview).not.toBeVisible()
