...
export function YouScreen() {
  const {
    athlete, settings, updateSettings, equipmentProfiles, setActiveEquipmentProfile, saveEquipmentProfile, history, exercises, sessions, surveys, deferredFeedback, records, mesocycles, historyMutations, cycleReviews, substitutionEvents, placementVerifications,
  ...
  const exportData = () => {
    const payload = createBackup({ athlete, settings, equipmentProfiles, history, exercises, sessions, surveys, deferredFeedback, records, mesocycles, historyMutations, cycleReviews, substitutionEvents, placementVerifications, activeMesocycleId, activeSessionId, onboardingComplete })
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' })
    const url = URL.createObjectURL(blob)
    const anchor = document.createElement('a')
    anchor.href = url
    anchor.download = `forgepath-backup-v16-${new Date().toISOString().slice(0, 10)}.json`
    anchor.click()
    URL.revokeObjectURL(url)
    setNotice('Verified version 16 backup created as open JSON, including per-movement placement, equipment-aware route generation, productive placement checks, recovery evidence, equipment profiles, records, plans, history changes, substitutions, and surveys.')
  }
    ...
    <div className="screen">
      ...
      <div className="settings-layout">
        <div className="settings-main">
          <section className="panel">
            ...
            <div className="placement-profile-evidence"><span><Sparkles size={17} /><span><strong>{athlete.entryRoute}</strong><small>{athlete.placement.confidence} confidence · {athlete.placement.ruleVersion} · {athlete.placement.decision}</small>{athlete.placement.selectedRoute !== athlete.placement.recommendedRoute && <small>Engine recommendation: {placementRouteLabels[athlete.placement.recommendedRoute]}</small>}</span></span><details><summary>Why and how this will be verified</summary><p>{athlete.placement.reasons.join(' ')}</p>{athlete.placement.uncertainInputs.length > 0 && <p><strong>Still uncertain:</strong> {athlete.placement.uncertainInputs.join(', ')}.</p>}<ul>{athlete.placement.verificationPlan.map((item) => <li key={item}>{item}</li>)}</ul><p><strong>Exit criteria:</strong> {athlete.placement.exitCriteria.join('; ')}.</p></details>
              {athlete.placement.movementPlacements && athlete.placement.movementPlacements.length > 0 && <div className="profile-movement-lanes"><p className="eyebrow">Movement-placement-v1</p>{athlete.placement.movementPlacements.map((movement) => <details key={movement.exerciseId}><summary><span><strong>{movement.exerciseName}</strong><small>{movement.family} · {movement.confidence} confidence</small></span><b>{placementRouteLabels[movement.selectedRoute]}</b></summary><p>{movement.reasons.join(' ')}</p><small>Skill {movement.movementSkill}/5 · heavy-work tolerance {movement.strengthTolerance}/5 · evidence {movement.dataConfidence}/5</small>{movement.uncertainInputs.length > 0 && <small>Unknown: {movement.uncertainInputs.join(', ')}</small>}</details>)}</div>}
        ...
        <aside className="settings-aside">
          ...
          <section className="panel"><div className="panel__header"><div><p className="eyebrow">System versions</p><h3>Diagnostics</h3></div><Database size={19} /></div><ul className="diagnostic-list"><li><span>App</span><strong>0.22.0 private alpha</strong></li><li><span>Rules</span><strong>0.22 per-movement starting lanes</strong></li><li><span>Calculations</span><strong>Placement v2 · Movement placement v1 · Placement verification v1 · Route session v3 · Volume v2 · PR v2 · Plan dose v1 · Muscle dose v1 · Equipment v1 · Load increment v1</strong></li><li><span>Backup schema</span><strong>Version 16</strong></li><li><span>Persistence</span><strong>Local v14</strong></li><li><span>Cloud sync</span><strong>Not connected</strong></li><li><span>AI provider</span><strong>Not required</strong></li></ul></section>
