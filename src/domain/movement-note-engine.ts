...
export function movementNoteError(note: unknown) {
  ...
  if (!(value.mesocycleId === null || typeof value.mesocycleId === 'string')) return 'Movement note training-block identity is invalid.'
  ...
  if (!(value.microcycleNumber === null || (typeof value.microcycleNumber === 'number' && Number.isInteger(value.microcycleNumber) && value.microcycleNumber >= 1))) return 'Movement note training-round number is invalid.'
