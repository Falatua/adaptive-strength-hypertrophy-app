...
Home Gym preference v2 adds a deliberate upper-body and posterior-chain bias. The Home Gym profile explicitly includes a deficit platform. Conventional deficit deadlifts, Romanian deadlifts, and stiff-leg deadlifts with a barbell rank as the preferred hinge builders, while barbell shrugs are the preferred direct trap accessory. In a normal three-session round, automatic support work reserves the ABX cambered-bar chest-supported row in two sessions and a pull-up exposure in the remaining session when time, equipment, pain, and athlete-approved block choices allow. This makes upper-back work present across the round without turning every session into an identical workout.

Home Gym preference v3 makes incline pressing the default pressing emphasis. A standard automatically generated round uses ABX incline pressing more often than general flat assistance when time and equipment permit. Competition Bench Press or another athlete-selected flat primary remains protected. Two-Board Press, Close-Grip Bench Press, and Spoto Press are purposeful flat exceptions for triceps development rather than the default pressing base. ForgePath selects one of those three for the training block and rotates the selection across new plan versions, keeping it stable inside the block. Each movement retains its own exact completed history, load, repetitions, RIR, pain, notes, and progression evidence.

JB's current pull-up capacity begins as an explicitly provisional 3 × 5 target. It is not written into completed history and cannot create a record, confidence increase, or progression claim. The first exact logged pull-up exposure replaces the estimate for later programming. Calves remain low priority: even if calves appear in declared automatic support regions, the Home Gym planner exposes no more than one calf opportunity per training round. The athlete may still choose calf work or any other movement directly.
