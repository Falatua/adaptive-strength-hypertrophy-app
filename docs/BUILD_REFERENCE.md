...
- `missed-opportunity-v3` adds `schedule-eligibility-v1`: automatic choices and athlete pins require an executable protected primary under the active equipment profile, irritating or avoid primary movements cannot lead, active placement pain gates stop automatic rebuilding, and unavailable or joint-flagged support work is removed from the first session with durable evidence.
...
- Version 22 open-JSON backup with validated schedule eligibility, missed-opportunity evidence and athlete priority pins, replay-validated plan-route and exact-movement checkpoint reviews, athlete-reviewed exact-history placement, per-movement placement, equipment-aware route-session provenance, placement decisions, productive verification and recovery evidence, expanded record definitions, celebration controls, equipment profiles, plan, review, history and catalog changes, substitution, survey-evidence, and deferred-feedback history, integrity checks, versions 1 through 21 migration, preview, restore, malformed-file rejection, and automatic pre-restore undo.
