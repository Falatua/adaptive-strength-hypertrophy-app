...
- `missed-opportunity-v2` adds an optional athlete-controlled next-session pin to the complete version 1 interruption evidence. A pin overrides only the first session, remains validated against the open queue, and leaves exact-exposure recency in control of the remaining order.
...
- Version 21 open-JSON backup with validated missed-opportunity evidence and athlete priority pins, replay-validated plan-route and exact-movement checkpoint reviews, athlete-reviewed exact-history placement, per-movement placement, equipment-aware route-session provenance, placement decisions, productive verification and recovery evidence, expanded record definitions, celebration controls, equipment profiles, plan, review, history and catalog changes, substitution, survey-evidence, and deferred-feedback history, integrity checks, versions 1 through 20 migration, preview, restore, malformed-file rejection, and automatic pre-restore undo.
