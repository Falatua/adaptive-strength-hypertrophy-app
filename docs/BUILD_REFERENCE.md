...
- `missed-opportunity-v4` retains `schedule-eligibility-v1` and adds `schedule-readiness-v1`: only answered pre-session evidence no more than 24 hours old can alter rebuilding; protective readiness removes optional fatigue, confirm readiness requires warm-up confirmation, reacclimation evidence changes the mode, fresh pain blocks mutation, and missing, skipped, or stale evidence remains unknown without penalty.
...
- Version 23 open-JSON backup with validated schedule eligibility and fresh-readiness evidence, missed-opportunity decisions and athlete priority pins, replay-validated plan-route and exact-movement checkpoint reviews, athlete-reviewed exact-history placement, per-movement placement, equipment-aware route-session provenance, placement decisions, productive verification and recovery evidence, expanded record definitions, celebration controls, equipment profiles, plan, review, history and catalog changes, substitution, survey-evidence, and deferred-feedback history, integrity checks, versions 1 through 22 migration, preview, restore, malformed-file rejection, and automatic pre-restore undo.
