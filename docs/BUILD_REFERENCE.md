...
- A 154-movement canonical built-in catalog plus six-match recommendations and searchable full-library workout substitutions, including a verified squat-to-leg-press path that preserves separate exact histories and prescriptions.
...
- `volume-progression-v1` turns the feedback the athlete already gives into next week's set count. Each muscle carries weekly landmarks for maintenance, minimum effective volume, the productive band, and the volume still recoverable, starting from published reference ranges and scaling with the athlete's own volume tolerance. Pump and target stimulus read as stimulus, end fatigue and pain as fatigue, and comparable exposures as performance. Low stimulus with manageable fatigue adds sets, holding performance adds one, strong stimulus under high fatigue holds, declining performance under high fatigue reduces, and reaching the recoverable ceiling or the final planned week deloads to the minimum effective volume.
