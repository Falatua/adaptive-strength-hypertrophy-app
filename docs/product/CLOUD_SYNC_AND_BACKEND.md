...
Private alpha 0.61.1 uses an authenticated cloud gate and automatic whole-state snapshot persistence. An invited athlete enters only the invited email and presses `Log in with email`. A new or signed-out browser opens the private time-limited confirmation link Supabase sends in the same browser profile. The renewable session is stored durably and then keeps the browser signed in through refreshes and app updates until sign-out, site-data removal, or server-side revocation. The athlete product has no password setup, password sign-in, or password recovery route. The request disables account creation, returns a generic response that does not reveal invitation membership, and routes verified links back to ForgePath. Public signup remains disabled. Email knowledge alone never establishes identity.
