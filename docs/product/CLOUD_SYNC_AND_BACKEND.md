...
The dedicated project is live at project reference `kdavpkphvapnckenbuyg` in AWS `us-east-2`. Its repaired migration ledger matches both committed migration files by SHA-256. A live read-only audit verified fourteen forced-RLS tables, two security-invoker views, zero normalized browser mutation grants, four intentional profile/device mutation grants, and an authenticated-only snapshot RPC. A rolled-back two-identity transaction verified device registration, projection-write denial, snapshot apply, idempotent replay, stale conflict preservation, state invariants, and cross-athlete invisibility. A follow-up query proved the rollback left zero test rows. Public signup is disabled. The private source repository holds only browser-safe project URL and publishable-key values as GitHub Actions secrets. The public build receives them only when `FORGEPATH_CLOUD_RELEASE_ENABLED` is exactly `true`; that repository variable remains unset pending one approved invitation and the real email-link and phone-to-laptop recovery drill. Database passwords and privileged keys were neither read nor stored.
