...
Private alpha 0.65.0 uses an authenticated cloud gate and automatic version 28 whole-state snapshot persistence, including athlete-approved training-block choices, direct exact-movement history, and the active equipment profile. Cloud restore re-merges the current system exercise catalog and untouched equipment templates before the state becomes active. That lets existing accounts receive the Freak Athlete Home Gym additions while preserving custom locations, preferences, completed history, and plans. No new Supabase table, RPC, grant, or Edge Function is required because the athlete-record shape and validated whole-state snapshot contract are unchanged. An invited athlete enters only the invited email and presses `Log in with email`, or submits with the phone keyboard Go action. A new or signed-out browser opens the private time-limited confirmation link Supabase sends in the same normal browser profile, not Incognito or an email-app preview. The renewable session is stored durably and then keeps Chrome or that browser signed in through refreshes and app updates until sign-out, site-data removal, or server-side revocation. On iOS, a newly installed Home Screen web app does not receive the default browser's local storage. ForgePath therefore lets the verified browser session create a random 100-bit, five-minute, single-use handoff code. An athlete who is already signed in in that browser can reopen the transfer page without requesting another email. Supabase stores only the code's SHA-256 digest; the installed app redeems it through a server-only function into its own renewable session, and the phone keyboard Done action can submit that code. A temporary server token-generation failure conditionally releases only its still-current redemption claim so the same unexpired code can be retried. Reusable access and refresh tokens never cross the clipboard, query string, or GitHub Pages. The sixty-second send cooldown persists through a page refresh or browser restart, and specific rate-limit guidance discourages replacement emails and states clearly that throttling is not an account lock. The athlete product has no password setup, password sign-in, or password recovery route. The request disables account creation, returns a generic response that does not reveal invitation membership, and routes verified links back to ForgePath. Public signup remains disabled. Email knowledge alone never establishes identity.
