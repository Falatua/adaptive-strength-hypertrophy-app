...
Private alpha 0.62.1 uses an authenticated cloud gate and automatic whole-state snapshot persistence. An invited athlete enters only the invited email and presses `Log in with email`. A new or signed-out browser opens the private time-limited confirmation link Supabase sends in the same browser profile. The renewable session is stored durably and then keeps the browser signed in through refreshes and app updates until sign-out, site-data removal, or server-side revocation. On iOS, a newly installed Home Screen web app does not receive the default browser's local storage. ForgePath therefore lets the verified browser session create a random 100-bit, five-minute, single-use handoff code. An athlete who is already signed in in that browser can reopen the transfer page without requesting another email. Supabase stores only the code's SHA-256 digest; the installed app redeems it through a server-only function into its own renewable session. A temporary server token-generation failure conditionally releases only its still-current redemption claim so the same unexpired code can be retried. Reusable access and refresh tokens never cross the clipboard, query string, or GitHub Pages. A sixty-second send cooldown and specific rate-limit guidance discourage replacement emails and state clearly that throttling is not an account lock. The athlete product has no password setup, password sign-in, or password recovery route. The request disables account creation, returns a generic response that does not reveal invitation membership, and routes verified links back to ForgePath. Public signup remains disabled. Email knowledge alone never establishes identity.
