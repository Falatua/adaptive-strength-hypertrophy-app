...
- Total requirement headings: 346.
- Sequential range: R-001 through R-346.
...
Private alpha 0.34.0 is the current verified implementation boundary. Evidence is recorded in [[Private Alpha Implementation 2026-08-10]], [[UX Audit 2026-08-10]], and [[Adaptive Strength and Hypertrophy App Build Bible#73. Cross-Device UX Audit and Task-Focus System]]. Version 0.34.0 adds predictable destination context, accessible onboarding progress, non-obstructive compact notices, an above-fold Today start path, Library discovery before calibration detail, and a safer active-workout completion hierarchy. Verification includes 191 deterministic tests across twenty files, forty-eight Playwright journeys across desktop and phone, a production PWA build, accessibility and static UI scans, and 390 by 844 visual review. All verified 0.33.0 and earlier training-engine evidence remains active, including `field-guide-synth-v1`, `schedule-priority-dose-v1`, backup schema version 24, and local persistence version 22. R-331 through R-340 and Chapter 72 capture the new hypertrophy policy and template source, but selectable failure-target double progression, per-exercise increments, and upper-lower templates remain unimplemented. R-341 through R-346 and Chapter 73 govern the verified cross-device audit changes. Personal readiness baselines, fixed-event pressure, downstream-fatigue interaction, complete later-session substitution, contextual preferences, companion progression, cloud sync, and the new hypertrophy-policy controls remain incomplete.
