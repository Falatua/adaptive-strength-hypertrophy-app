...
Private alpha 0.64.0 makes the validated version 28 snapshot cloud-authoritative for invited accounts, including athlete-approved training-block choices and direct exact-movement history entered from the Exercise Library, adds persistent safe update notification, preserves the newest unconfirmed snapshot through a browser or operating-system kill, explicitly stores the renewable Auth session in durable browser storage, and securely establishes a separate renewable session in the installed iOS Home Screen app after the default browser verifies the invited email. Version 27 migrates without inventing historical performances. Direct history uses the existing snapshot RPC and requires no new Supabase migration, browser grant, or Edge Function. Normal phone Chrome keeps its own renewable session; login and handoff forms support keyboard submission, and the email-send cooldown survives a Chrome refresh or restart. An already-verified browser can create another Home Screen code without sending another email. A dedicated remote project exists in a separate approved Supabase organization: `ForgePath`, project reference `kdavpkphvapnckenbuyg`, AWS `us-east-2`. JB-OS and Roman TD Global Leaderboard were not modified, paused, deleted, or reused.
