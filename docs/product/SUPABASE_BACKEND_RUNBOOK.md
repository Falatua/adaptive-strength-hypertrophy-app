...
Private alpha 0.65.0 keeps the validated version 28 snapshot cloud-authoritative for invited accounts, including athlete-approved training-block choices, direct exact-movement history, and the active equipment profile. The app re-merges the current system exercise catalog and untouched system equipment templates when a cloud snapshot is restored, so existing accounts receive the Freak Athlete Home Gym additions without losing athlete-owned profiles, preferences, completed training, or plans. This catalog upgrade changes no athlete-record shape and requires no new Supabase migration, grant, RPC, or Edge Function. Persistent safe update notification, interrupted-save recovery, renewable browser sessions, and the secure installed iOS Home Screen handoff remain unchanged. Normal phone Chrome keeps its own renewable session; login and handoff forms support keyboard submission, and the email-send cooldown survives a Chrome refresh or restart. A dedicated remote project exists in a separate approved organization: `ForgePath`, project reference `kdavpkphvapnckenbuyg`, AWS `us-east-2`. JB-OS and Roman TD Global Leaderboard were not modified, paused, deleted, or reused.
...
- a live `pwa-handoff` Edge Function that stores only a SHA-256 digest, requires a recently verified session to create a 100-bit code, atomically redeems it once within five minutes, and returns a server-generated token hash only to the approved app origin; the newer structured-error and retry-safe token-failure source remains pending dashboard deployment;
