...
Private alpha 0.60.0 makes the validated version 25 snapshot cloud-authoritative for invited accounts and adds persistent safe update notification. A dedicated remote project exists in a separate approved Supabase organization: `ForgePath`, project reference `kdavpkphvapnckenbuyg`, AWS `us-east-2`. JB-OS and Roman TD Global Leaderboard were not modified, paused, deleted, or reused.
