...
Private alpha 0.61.0 makes the validated version 26 snapshot cloud-authoritative for invited accounts, adds persistent safe update notification, and preserves the newest unconfirmed snapshot through a browser or operating-system kill. A dedicated remote project exists in a separate approved Supabase organization: `ForgePath`, project reference `kdavpkphvapnckenbuyg`, AWS `us-east-2`. JB-OS and Roman TD Global Leaderboard were not modified, paused, deleted, or reused.
