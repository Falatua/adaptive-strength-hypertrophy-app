...
test('builds an explainable multi-dimensional placement and preserves athlete control', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText('high confidence', { exact: false })).toBeVisible()
  ...
  await expect(page.getByText(/high confidence.*placement-v1.*conservative/i)).toBeVisible()
  ...
  expect(persisted.state.mesocycles.find((plan: { id: string }) => plan.id === persisted.state.activeMesocycleId)).toMatchObject({ dominantAdaptation: 'reacclimation', title: 'Base-Building Cycle · Starting Cycle' })
  const dimensions = await page.evaluate(() => ({ scrollWidth: document.documentElement.scrollWidth, clientWidth: document.documentElement.clientWidth }))
  expect(dimensions.scrollWidth).toBeLessThanOrEqual(dimensions.clientWidth)
...
test('turns warm-up, first-set, session, and recovery evidence into an auditable placement check', async ({ page }, testInfo) => {
  ...
  await page.getByRole('button', { name: 'Save feedback & finish' }).click()
  await page.getByRole('button', { name: 'Today' }).click()
  await expect(page.getByText('How did you recover from Bench Strength + Upper Body?')).toBeVisible()
