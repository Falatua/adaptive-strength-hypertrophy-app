...
- Set-entry autofill: Set 1 acts as the functional template for untouched later straight sets in the same movement. The interface proves the behavior through filled editable rows and separate `Log set` actions without repeating an explainer above every set table. Manual exceptions, completed or skipped work, structured technique rows, and angle ladders remain unchanged. A visible prescription is not entered evidence; actual RIR stays unknown until the athlete enters or accepts that field.
- Bodyweight load mode: eligible movement cards use one compact two-state control for `Bodyweight` and the athlete's active unit. Bodyweight rows show `BW` in the load column, retain repetitions and effort inputs, and never present a zero-pound field as the meaning of bodyweight work.
- Workout preview: Today places `Preview workout` beside the dominant start path. Plan gives each queued session a written `Preview` action. Both routes open the same read-only movement list with planned sets, repetitions, load mode, latest exact exposure, and one progress cue.
...
The active workout places one compact evidence panel after the three-column movement context. Its hierarchy is fixed: suggestion and confidence, Last / Today / Next, What earns it, source-set and unknown-input counts, then a written display-only boundary. It may not compete with the set table or hide the current prescription. Evidence blue marks explanation and orange marks a protective hold. The panel never includes a bulk-edit action. It explicitly states that entered load, repetitions, and RIR decide what is recorded.
...
The blueprint header includes a secondary `Preview full block` action beside the existing edit action. It opens a read-only dialog with three high-level facts, current round, approximate training weeks left, and expected final review, followed by a horizontally scrollable round route. Recorded, current, planned, and recovery-review states use written labels in addition to color. On compact phones, the facts stack, the route scrolls within its own boundary, explanatory text wraps, and the dialog never expands the page horizontally.

The lower outlook separates progression from deload timing. Progression shows the load, repetitions, then sets order and states that exact future targets remain unknown until the prior round review. Deload timing says `No fixed deload date` unless current evidence recommends recovery, identifies the planned final decision point, and explains that an earlier recovery proposal may arise from pain, fatigue, technique, or unresolved work. Opening or closing this preview is presentation-only and cannot mutate the plan.
...
Inside an active workout, `Change` opens one scope-first replacement flow. Two equally explicit choices, `This workout` and `Entire training block`, update the written consequence before the athlete browses candidates. Selecting a candidate does not mutate immediately. The final comparison and action repeat the chosen scope. A block-scoped change applies to the active workout and creates a new block version for future planned sessions while preserving completed work and exact histories. Main-lift protection remains an additional confirmation, not a substitute for the scope explanation.

Bodyweight-capable movement cards keep the load-mode control immediately above the set table. The selected mode applies to unfinished sets only, uses `aria-pressed`, and leaves completed evidence immutable. The progress cue names the prior single-set or session-total record and the exact repetition threshold that surpasses it. Calibration and bridge routes show an honest baseline or build-toward cue instead of hiding gamification; protect and pain-aware states retain a written pause reason.
