# ForgePath Private Alpha 0.68.0
...
Private alpha 0.68.0 gives Traps its own Library body part, anatomical emblem, direct muscle-dose lane, programming priority, upper and pull-day membership, and curated movement base. Existing Barbell Shrug history and preferences stay on the same stable ID while five new canonical shrug variations expand the catalog to 248 movements. Backup schema remains 28, local persistence advances to 27 for preservation-safe catalog migration, and the Supabase cloud-authoritative data contract is unchanged.

The public source repository includes a [complete product-specification snapshot](docs/product/README.md) containing the Build Bible, all 445 requirements, traceability matrix, verified implementation status, cross-device and functional UX audits, hosting contract, exercise-library and recommendation specification, cloud-sync and backend specification, longitudinal product simulation audit, and pixel training-adventure specification. Obsidian remains the editable source of truth.
